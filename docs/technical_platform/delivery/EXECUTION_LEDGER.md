...
## 2026-09-18 多客户端进程资源门禁

- 缺陷定义：原 `ResourceLocks` 仅在单进程线程间计数，两个同时启动的客户端可各自取得 Office、同一输出目录或第三个模型调用资源，存在并发写入、Office COM 冲突和超出客户端模型并发上限的风险。
- TDD 先以独立 Windows `spawn` 进程复现：新增接口前持锁子进程因不支持共享锁目录而红灯；随后为正式 `CLIENT_RESOURCES` 增加用户临时目录下的跨进程槽位锁。锁文件名仅保存资源名 SHA256 和槽位号，不写业务路径；Office 与同一输出目录容量为 1，模型容量为 2，等待期间仍响应取消。
- 跨进程专项 8/8 通过：第二个 Office/同目录输出进程等待，两个模型进程可同时进入而第三个等待；锁目录建立失败会释放进程内预占，不遗留假忙状态。资源锁/生成/浏览器循环/调度邻近 15/15 通过，限定 Ruff 与目标 Mypy 通过；客户端及技术平台完整回归 1126 passed（387.46 秒）。
...
| G01-01 | passed | 拆分版本化程序、用户数据、缓存、凭据路径；记住项目/数据根，启动不反复要求选目录；缺盘时明确提示不偷偷回落C盘。 | 正常启动只读取项目位置索引，不再要求选择统一工作文件夹；项目资料/成果留在用户选定项目目录。浏览器资料、凭据、下载与更新暂存均经 `StorageLayout` 使用已批准非系统盘目录，缺盘明确失败且不重建、不回落 C 盘；迁移须确认、关闭消费者并取得跨进程租约。存储/浏览器Profile/凭据/更新链专项139项及完整客户端/平台1126项通过。 |
...
| G04-03 | passed | 客户端实际HTTP model2、DETAIL生成子进程office1、生成目录排他及等待取消已接；在途HTTP不因caller取消释放。服务端审核执行器固定单任务，标签排他已测。 | 正式 `CLIENT_RESOURCES` 采用跨进程槽位锁：独立进程验证 Office 与同一输出目录容量1、模型容量2及第三个等待，异常建锁不遗留本地预占；专项8项、邻近15项及客户端/平台完整1126项通过。本机 Office+WPS 并存真实生成退出码0、来源/模板哈希不变、95 sheet门禁和1,322个汇总公式保护通过，证据 `D:/ZQ-Acceptance/office-wps-real-acceptance.json`。隔离 PostgreSQL 验证预占幂等、超额阻止和 capture 单扣，证据 `D:/ZQ-Acceptance/postgresql-wallet-concurrency.json`。 |
...
| G08-02 | passed | CI跑客户端无GUI单测、服务端契约/数据库迁移、静态检查、安全负面测试；Windows job构建客户端，Linux job构建服务端镜像；离线/mock测试不消耗真实API。 | 最终候选提交 `66c309b...`：客户端 run `35282965110` 的回归与 Windows build 均 success，服务端 run `35282965119` success；候选 artifact `10524106705` 已生成；Release 资产提升 run `35285955239` success。 |
| G08-03 | passed | 新增签名清单、包哈希、协议/schema范围、平台架构、可信key ID和发布说明；私钥放受控签名环境/GitHub受保护Secret，不进服务端普通变量或仓库；建立轮换/撤销机制。 | `v0.2.8` sequence 3 已用用户授权且仅存于 `D:/1/KEY` 的私钥离线签名，并用客户端内置公钥复验；清单 SHA256 `845d50d7...`，普通包 SHA256 `73f8b063...`，平台 `windows/x86_64`，协议 1，schema 11，key ID `zq-release-20260917`。轮换/撤销负测及发布说明已在先前回归和 `RELEASE_0_2_8.md` 验证。未声称具备 Authenticode。 |
...
| G08-06 | doing | 发布到GitHub Release或经确认HTTPS分发源；包名和版本不可变，旧URL不覆盖不同内容；私有源采用安全下载机制，客户端无仓库PAT。 | `v0.2.8` GitHub 草稿 Release 已精确指向 `66c309b...`，7项资产均为 uploaded；GitHub 返回的普通包 `73f8b063...`、托管包 `5e9b1a78...`、签名清单 `845d50d7...` 等 digest 与本地一致。发布工作流只允许指定 CI 的四项产物；仍待生产部署、灰度验收后公开 Release。 |
...
| G09-05 | doing | 先隔离环境验证新服务与旧客户端/新客户端，确认授权与费用后跑最小真实模型调用；再备份生产并部署指定提交，检查构建SHA、迁移head、健康、登录、总控、模型、任务、取消与账务。 | 历史提交的 health、capabilities、迁移head、总控及发布能力已验证；最终候选 `66c309b...` 的服务端 CI 已通过但生产尚未部署。Zeabur OAuth 当前要求用户重新登录；部署精确SHA后再执行账号001的成功/取消/账务真实模型验收，累计费用上限20元。 |
...
| G10-03 | doing | 用验收的同一提交重建EXE及压缩包；保留老稳定版，不覆盖运行中的EXE；启动截图、文件版本、资源清单、SHA256、签名与Git标签一致。 | `v0.2.8` 精确提交 `66c309b...` 的 Windows CI 已通过；普通包 `73f8b063...`（252,777,343 bytes/3,881 entries）和托管包 `5e9b1a78...`（336,449,001 bytes/3,886 entries）与元数据、GitHub digest一致。隔离EXE健康探针验证版本0.2.8、schema11、协议1、WebEngine、登录窗口及内置Skill/模板哈希。签名清单已复验；仍待部署、灰度后公开标签/Release再最终签收。 |
| G10-04 | doing | 灰度机器从旧版实际更新到新版本；验证登录、历史、分支、真实流式、取消、浏览器密码和成果；成功后再将该已验收版本设stable。 | 线上总控当前实际显示 `v0.2.7` 为 stable、`v0.2.6` 为 withdrawn；该状态与早期账本记录不同，已按现场证据纠正。因0.2.7不含最终跨进程门禁，不作为最终交付候选。`v0.2.8` 尚未登记/灰度；总控提交时管理员会话过期，未虚报成功。待001真实流式/取消/账务、托管升级、历史/分支、浏览器密码和成果复核完成后才允许把0.2.8激活stable。 |
...
## 2026-09-18 v0.2.9 signed-update and paid-acceptance closure

- The managed-updater transition failure was reproduced and narrowed to archive entry-point discovery: the Windows package contained both `ZQ Workspace.exe` and `QtWebEngineProcess.exe`, while the installed v0.2.7 updater intentionally accepted exactly one executable candidate. Commit `84d6107131069aee33f758626fd5c863809da99b` implements the compatible transition bridge: the WebEngine helper is stored as `QtWebEngineProcess.pending` in the archive and restored to the live executable name by the updater during installation. The application executable remains the only pre-install `.exe` candidate.
- A local packaging attempt was rejected rather than released. Invoking `scripts/package_technical_platform.py --help` unexpectedly ran packaging because the script does not parse a help-only mode; it consumed a stale local `dist` tree and produced an archive containing an old 0.2.7 executable. The smoke test failed at the client-version assertion. Those local archives were never uploaded or activated. The GitHub Actions artifact built from the immutable `84d610...` candidate is the authoritative v0.2.9 package.
- The complete local regression passed 1436 tests: report-review client 227, server 206, technical-platform 901 and platform-update 102. Immutable-candidate GitHub runs `35292154033` (server) and `35292154030` (Windows client) both completed successfully.
- Public release `v0.2.9` contains `ZQ-Workspace-0.2.9-Windows.zip`, size `252778667`, SHA-256 `3484b2d3dab52d3956cc3c36270d4f8b6b43684c8d28b2846ef0692b8c457654`. All eight independently downloaded byte ranges reassembled to the exact size and digest, `ZipFile.testzip()` passed, and the archive contained one `.pending` WebEngine helper and no second live `.exe`. Manifest SHA-256 is `d1a4e634a5dd24e6bf606b51585304d9a1bf38edd373972abde37b38cc3cc8d5`; application-level Ed25519 verification passed with embedded key id `zq-release-20260917`. No Authenticode certificate has been configured, so this evidence must not be represented as Authenticode signing.
- The real v0.2.7 updater installed the downloaded v0.2.9 package in an isolated managed installation and returned `status=succeeded`. Post-upgrade verification confirmed journal version 0.2.9/sequence 4/idle phase, unchanged business-database hashes and inventory, preserved project/main-session/child-branch/history/artifact records, a decryptable unchanged DPAPI credential, and restoration of the WebEngine helper from `.pending` to `.exe`. The managed application executable correctly rejects direct launch outside the installation launcher; the updater's native update-healthcheck completed before success was committed.
- Commit `29c123a8a1bfd9316aef789ff3117c6cbc4e0746` adds an admin price-only update path so accepted per-million-token rates can be saved without re-entering or replacing the provider API key. Focused TDD verification passed 17 tests, Ruff passed and the complete server suite passed 207 tests with 1 skipped. GitHub server run `35297228965` completed successfully; the corresponding Windows client run `35297229005` was still in progress at this ledger entry and must be updated before the repository green gate is claimed.
- Zeabur capability metadata reported exact server build `29c123a8a1bfd9316aef789ff3117c6cbc4e0746`. The accepted model rates were saved as input 0, output 8.64, cache hit 0.0432, cache miss 2.16 and reasoning 0 RMB per million tokens. No provider secret was copied into the ledger, client, package or test artifact.
- Paid model acceptance used only synthetic text. Review job `2ac16896-93b3-43d4-a1d9-109036f26562` completed successfully; the visible account balance changed from 98.05 to 98.03 RMB, a 0.02 RMB charge and well within the explicitly authorized 20 RMB cumulative ceiling. A preceding tiny successful request produced no visible cent-level balance change and was retained only as insufficient evidence, not counted as the billing gate.
- After package integrity, real updater, preservation and paid-billing gates passed, admin activation promoted `v0.2.9` to `stable` at sequence 4. The public current-release endpoint independently returned version 0.2.9, sequence 4, the exact manifest/package hashes and the signed GitHub Release download URL. Earlier 0.2.7 was withdrawn; 0.2.8 remains canary.
- Independent clean-machine verification remains unavailable because no separate clean Windows computer or virtual machine was provided. Therefore the complete G11 clean Windows / Office / WPS installation matrix is still unverified and must not be marked complete. All locally available and online acceptance gates above are complete; this external environment gap is the only recorded release-acceptance remainder.

## 2026-09-18 v0.2.10 managed-launch hotfix candidate

- Immediate post-release user evidence showed the installed client raising `Obsolete managed executable; use the installation launcher`. Byte-level inspection proved this was not user error: two tracked Python path literals had already been irreversibly replaced with Unicode replacement characters. The current active executable could therefore never equal the expected path and was falsely classified as obsolete. The same damaged literal also affected the independent updater's baseline-initialization check.
- Tests were changed first to use the real `ZQ\u6280\u672f\u5e73\u53f0` directory and executable names. The initial focused run failed exactly at active-client identity, stable-launcher selection after WebEngine restoration, old-shortcut handoff and updater initialization. This is retained as the reproducible red phase.
- The hotfix now derives the exact active executable from one escaped, encoding-stable product identity rather than recursively counting all `.exe` files. This also prevents a restored `QtWebEngineProcess.exe` helper from being misclassified as a second main application. Launching the current executable proceeds normally; launching an older managed executable securely starts the stable installation launcher and exits without opening stale code. The updater imports the same identity for baseline initialization.
- A replacement-character scan across `src`, `scripts` and `tests` returned no remaining affected files. Focused update/release coverage passed 116/116, Ruff passed, and the complete local regression passed 1438 tests with 1 skipped in 506.55 seconds. Client metadata is advanced to v0.2.10, sequence 5. GitHub CI, signed package construction, real managed-upgrade verification, admin activation and online stable-release verification remain required before this candidate may be described as released.
- The first v0.2.10 CI package was deliberately rejected before publication after a deeper real-upgrade probe. Although its main executable and data-preservation checks passed, the legacy root launcher still exited because the candidate health check renamed `QtWebEngineProcess.pending` back to `.exe`, recreating a second executable visible to that older launcher's one-EXE rule. The draft GitHub Release and all seven draft assets were deleted; the tag was never created, the release was never public and the server registry was never activated.
- The replacement design keeps the signed helper under its `.pending` name and sets Qt's supported `QTWEBENGINEPROCESS_PATH` to that exact path before importing WebEngine. It performs no runtime rename, so an existing legacy launcher continues to see exactly one main executable on every launch. Development trees that contain the normal `.exe` helper remain supported. The revised red/green runtime, launcher, packaging, updater and release coverage passed 117/117 and Ruff passed. A new immutable CI candidate and full real-upgrade/launcher/browser verification are required.
- Candidate commit `67a165d9dc8d2984df5181ed5e4f5e3593584962` was not promoted: server CI run `35303773494` passed, but Windows client run `35303773498` failed 23 Qt WebEngine browser probes after 1209 tests passed. The failure was reproduced locally only when `test_runtime_files.py` preceded the browser probes. The production helper selector wrote a temporary pytest path into the process-global environment while the test's absent-key cleanup did not restore it, so later WebEngine subprocesses inherited a stale path. This was test-process pollution rather than evidence that the packaged runtime design failed. The selector now accepts an injectable environment mapping; production still defaults to `os.environ`, while tests use an isolated dictionary. The exact formerly failing order now passes 19/19, the update/release group passes 117/117 and the limited Ruff gate passes. A fresh immutable commit, green CI package and real managed-launch/browser acceptance are still required before v0.2.10 can be published or activated.
- The first post-fix full regression used an accidentally over-nested evidence root and failed five WebEngine tests because Chromium could not create its cache below the resulting long Windows path. No assertion was suppressed: the four affected files passed 13/13 with the normal short D-drive pytest root, and the complete acceptance runner then passed 227 report-review client, 208 server, 901 technical-platform and 104 platform-update tests (1440 total). Authoritative evidence is `D:/ZQ-Acceptance/r-effd0414b3484c0ea2a3ac9722b48a58`; the long-path run is retained only as a rejected harness invocation, not counted as a product pass.
- Immutable candidate `e22c10e98473e4d578d2134fb443fdb9776c366f` passed GitHub server run `35305942221` and Windows client run `35305942209`. The promoted Windows package is 252,778,813 bytes with SHA-256 `f0631d9b9dea7ff946b5592bd85a38036078bb98e8bb14f2f905de28db5a3896`; the eight independent ranges, GitHub asset digest, CI metadata, ZIP integrity, one-main-EXE inventory, pending helper and frozen executable health check all matched. The signed manifest is 689 bytes with SHA-256 `f5be99cde6d93e33c1f4781f741146483593ab85ca33f965a25e4f113d282c8e` and verified against the embedded `zq-release-20260917` public key. No Authenticode certificate is configured.
- A fresh isolated v0.2.9 managed installation was built with synthetic project, main session, child branch, history, artifact, two database records and one DPAPI credential. Its frozen old updater installed the final signed v0.2.10 package successfully. Post-upgrade checks confirmed sequence 5/idle, unchanged database fingerprints/inventory and all synthetic history/artifact/credential state, a retained `.pending` WebEngine helper, exactly one application `.exe`, and an old root launcher that remained running instead of raising the obsolete-executable error. Evidence roots are `D:/ZQ-Acceptance/managed-upgrade-final2-0210` and the corresponding `upgrade-final2-*` files.
- GitHub Release `v0.2.10` was published only after those gates passed. The admin registry accepted the same signed manifest, then activated v0.2.10 as stable and withdrew v0.2.9. A fresh public `GET /api/v1/client-releases/current` returned stable version 0.2.10, sequence 5, manifest SHA-256 `f5be99c...82c8e` and package SHA-256 `f0631d9...a3896`. The independent clean-Windows Office/WPS matrix remains unavailable and is still the only external G11 acceptance gap.
