...
| G04-01 | doing | 多会话后台执行、重命名/归档/恢复、运行/未读、草稿隔离、指定消息分叉/返回来源UI已接。schema 5/6持久元数据及草稿；项目树与完整分支上下文待实施。 | 会话数据层及窗口专项；完整781；本轮检查点证据 |
| G04-02 | doing | 分叉记录来源消息摘要及分叉前已成功任务的结果摘要引用；读取重新核对归属/时间/摘要，不复制执行动作或授权。成果文件固定版本核验、后代继承及Agent消费未完成。 | 结果变化拒绝、账号/时间隔离专项通过；完整783，不据此通过整项 |
| G04-03 | doing | 客户端实际HTTP model2、DETAIL生成子进程office1、生成目录排他及等待取消已接；在途HTTP不因caller取消释放。服务端/跨进程/标签/Office实例退出验证待完成。 | 23关联；完整754通过，r-20cdd3d0652e451780908da4f5444960；未开放UI并发 |
...
| G08-04 | pending | EXE与资源完整包及独立updater；后台下载、验签、等待空闲、迁移前备份、切换、健康检查和回退。签名更新不等于Windows代码签名，Authenticode有证书再配置，不声称已具备。 | 待验证 |
| G08-05 | pending | 服务端新增客户端发布查询/管理和总控页面：draft/canary/stable/withdrawn、灰度、管理员鉴权、审计；只引用已验证签名包，管理员不能临时绕过签名发布。 | 待验证 |
| G08-06 | pending | 发布到GitHub Release或经确认HTTPS分发源；包名和版本不可变，旧URL不覆盖不同内容；私有源采用安全下载机制，客户端无仓库PAT。 | 待验证 |
| G08-07 | pending | 使用含历史项目的测试数据完成N→N+1→N+2及一次失败恢复；数据、附件、外部Skill、记忆与密码库保留；旧程序无更新器时提供一次性保留数据的过渡安装包。 | 待验证 |
| G09-01 | pending | 更新服务端Docker构建清单/requirements/部署文档，共享契约可导入；桌面WebEngine/Office依赖不装入Linux服务镜像。 | 待验证 |
| G09-02 | pending | 完成数据库增量迁移：任务/事件/幂等/租约/发布元数据；必要索引、唯一约束和清理策略；备份恢复演练；单执行者迁移，首版保持一个服务副本直到扩容验收。 | 待验证 |
| G09-03 | pending | 环境变量逐项与代码定义一致，保留现有JWT及供应商加密密钥；所有新增变量提供校验/default说明；生产缺关键值fail-closed，不使用开发回退。 | 待验证 |
| G09-04 | pending | 配置事件流的代理/应用超时与连接恢复；数据库、任务队列、过期hold/临时正文清理、发布下载源健康指标；日志仅脱敏ID和错误分类。 | 待验证 |
| G09-05 | pending | 先隔离环境验证新服务与旧客户端/新客户端，确认授权与费用后跑最小真实模型调用；再备份生产并部署指定提交，检查构建SHA、迁移head、健康、登录、总控、模型、任务、取消与账务。 | 待验证 |
| G10-01 | pending | 在已核实的发布仓库使用codex/前缀分支逐阶段提交，CI通过后按保护规则合并；不force-push，不顺手提交无关变动；OA有必要变更则单独提交/发布记录。 | 待验证 |
| G10-02 | pending | 先部署兼容新旧客户端的服务器，确认就绪后才激活客户端更新通道；若推送即自动部署，发布前控制触发分支/窗口，不能在未就绪main上试错。 | 待验证 |
| G10-03 | pending | 用验收的同一提交重建EXE及压缩包；保留老稳定版，不覆盖运行中的EXE；启动截图、文件版本、资源清单、SHA256、签名与Git标签一致。 | 待验证 |
| G10-04 | pending | 灰度机器从旧版实际更新到新版本；验证登录、历史、分支、真实流式、取消、浏览器密码和成果；成功后再将该已验收版本设stable。 | 待验证 |
| G10-05 | pending | 生产和客户端回退演练使用隔离副本，不对真实生产随意降库；线上故障先撤回新更新通道并按兼容矩阵处理，不能旧EXE直接打开不兼容新库。 | 待验证 |
...
| NL07-03 | G02/G08/G09/G10/G11 | pending | 增  /  scripts/run_agent_e2e.py  /  从输入消息到理解、确认、执行、验证、成果的本地端到端测试  / 
