"""Check packaged rules and the login window; close only this test process."""

import os
import subprocess
import tempfile
import time
from pathlib import Path

import win32gui
import win32process

ROOT = Path(__file__).resolve().parents[1]


def main(root: Path | None = None):
    root = (root or ROOT).resolve()
    folder = Path(
        os.environ.get(
            "TP_SMOKE_DIST", root / "dist/technical_platform/ZQ技术平台"
        )
    )
    rules = folder / "_internal/asset_based_agent/technical_platform/review_rules.txt"
    assert (
        rules.read_bytes()
        == (
            root / "src/asset_based_agent/technical_platform/review_rules.txt"
        ).read_bytes()
    )
    from asset_based_agent.technical_platform.local_migrations import SCHEMA_VERSION
    from asset_based_agent.technical_platform.release_info import CLIENT_VERSION
    from asset_based_agent.technical_platform.updates.installer import probe_candidate

    health_root = root / 'build/packaged-health-smoke'
    health_root.mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(prefix='candidate-', dir=health_root) as temporary:
        info = probe_candidate(folder / 'ZQ技术平台.exe', Path(temporary), None)
    assert info['client_version'] == CLIENT_VERSION
    assert info['local_schema_version'] == SCHEMA_VERSION
    assert info['protocol_version'] == 1
    assert info['webengine_import'] is True
    launch_root = root / 'build/packaged-health-smoke/launch-data'
    launch_root.mkdir(parents=True, exist_ok=True)
    launch_env = dict(os.environ, ZQ_SETTINGS_ROOT=str(launch_root))
    process = subprocess.Popen([str(folder / "ZQ技术平台.exe")], cwd=folder,
                               env=launch_env)
    try:
        deadline = time.monotonic() + 60
        while time.monotonic() < deadline:
            titles = []

            def visit(hwnd, _, titles=titles):
                if win32process.GetWindowThreadProcessId(hwnd)[1] == process.pid:
                    titles.append(win32gui.GetWindowText(hwnd))
                    if win32gui.GetWindowText(hwnd) == "Unhandled exception in script":
                        win32gui.EnumChildWindows(
                            hwnd,
                            lambda child, _: titles.append(
                                win32gui.GetWindowText(child)
                            ),
                            None,
                        )

            win32gui.EnumWindows(visit, None)
            if "Unhandled exception in script" in titles:
                raise RuntimeError(repr(titles))
            if any("登录" in title for title in titles):
                print(
                    'PASS: packaged health, rules and login window verified; '
                    f'client={CLIENT_VERSION}; schema={SCHEMA_VERSION}'
                )
                return
            if process.poll() is not None:
                raise RuntimeError(f"Executable exited: {process.returncode}")
            time.sleep(0.5)
        raise RuntimeError(
            f"Login window not observed within 60 seconds; windows: {titles!r}"
        )
    finally:
        if process.poll() is None:
            process.terminate()
        process.wait(timeout=10)


if __name__ == "__main__":
    main()
