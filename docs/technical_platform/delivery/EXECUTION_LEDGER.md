...
| G04-03 | doing | 客户端实际HTTP model2、DETAIL生成子进程office1、生成目录排他及等待取消已接；在途HTTP不因caller取消释放。服务端审核执行器固定单任务，标签排他已测。 | 新增明确容量门禁：客户端同时两个model租约可进入且第三个等待；服务端两个不同任务由单工作线程串行，24项资源锁/审核任务专项通过。Office/WPS退出、跨进程锁及真实PostgreSQL钱包竞态仍待环境验证，暂不签收。 |
