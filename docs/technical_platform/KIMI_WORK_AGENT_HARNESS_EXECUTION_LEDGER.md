...
### G11 后续修复（2026-09-19，本地提交）：生成类 Skill 二次确认弹窗与多成果漏路由

- 用户实机反馈（完全访问权限模式）：请求"生成评估明细表和两年一期财务简报"时，Agent 只路由一个 Skill，并弹出"确认生成资料及授权"模态框要求手动指定三期报表角色+勾选同意，助手甚至声称"财务简报在当前能力目录中没有对应的生成工具"。
- 根因（代码核实，非推测）：
  1. `execute_plan` 对所有 GENERATORS 无条件弹 `GenerationDialog`，完全无视三档权限模式——而权限策略表本就规定 `generate_file` 仅在"请求批准"模式询问（副本生成、原件只读哈希校验，非危险操作）。
  2. `financial-brief-docx` 被刻意排除在规划候选之外（`capability_registry`，因已部署 Zeabur 服务端仍为旧协议，本轮不能向其发送未知技能类型），模型目录看不到它 → 纯关键词捷径 `try_local_builtin` 劫持含"财务简报"的请求，只路由财务简报一个 Skill，"明细表"需求被静默丢弃。
- 修复（测试先行，7 项新测试首轮 6 failed 后全绿）：
  1. `app.py execute_plan`：生成确认改为权限模式感知——"请求批准"模式保留原对话框；"帮我批准/完全访问权限"模式跳过弹窗，`generation.py` 新增 `auto_generation_roles`/`select_financial_reports`/`financial_report_date` 自动绑定角色（财务简报按资产负债表表头日期从选定资料中筛出三期报表，银行流水等非报表自动排除；识别不足/超出时在对话中说明原因而非弹窗）。事件文案区分"直接执行"与"仍需独立校验"。
  2. `app.py try_local_builtin`：含"财务简报"的请求同时识别"明细表/历史沿革"诉求，组成本地技能链依次执行（首步成功后 `finished` 钩子自动续跑下一步；首步取消/失败则终止并在对话说明）；服务端协议保持不变，财务简报仍不进规划候选（服务端未升级前的既定约束）。
- 验证：`test_local_generation_direct.py` 7 passed（含完全/帮我批准免弹窗、请求批准保留弹窗、5 文件自动收窄三期、缺报表对话说明、明细表→简报链式续跑、取消即终止）；路由/生成/契约邻域 50+40 passed；全量分批 555+170+473+554=1752 passed、1 skipped（根目录 4 个开工前既有收集错误文件照旧排除）；ruff 变更文件通过。
- 边界：请求批准模式行为不变；REVIEW 送审范围确认不变；锁定模板/来源哈希/授权持久化链路不变。未推送、未部署；冻结包未含本修复（下次构建随包）。

### G11 后续构建（2026-09-19）：含生成 Skill 免弹窗修复的候选包 20260919-092615

- 主 EXE SHA256 `866d9000…d940e4`（16,607,825 B）；bootstrap 两个 onefile 单独补建，
  哈希见该目录 ACCEPTANCE-RECORD.md；file-manifest.json 4730 个文件。
- 冒烟 PASS（client=0.2.10, schema=11）；冻结包扫描 0 命中。
- 本包为当前候选，取代 20260919-071611；历史候选包均未覆盖。
- 免弹窗直执行与链式续跑的实机复测待用户在本包上进行。

### G11 后续实机验收（2026-09-19）：候选包 20260919-121422（当前候选）

- 主 EXE SHA256 `84e4b3f1…17ff21a`（16,608,126 B）；含 094774f 免弹窗/链式路由 + 4ac7c04 成功反馈文案修复。
- 实机 GUI 驱动（pywinauto，离线零费用）三轮：完全/帮我批准模式免弹窗直生成财务简报，
  5 文件自动收窄三期报表，docx/pdf/png 产出且 PNG 目视正确；请求批准模式弹窗保留且取消不落任务；
  最终包复验通过。证据 D:\ZQ-Acceptance\real-machine-test\20260919-dialogfix\。
- 冒烟 PASS；冻结扫描 0 命中；全量 1752 passed / 1 skipped。
- 明细表步骤的真实模型链式端到端属付费调用，按密件约定未执行，待用户单独授权。
- 未推送、未部署、未发布。

### G11 后续实机验收（2026-09-19）：候选包 20260919-151211 真实资料端到端通过（当前候选）

- 主 EXE SHA256 `1681b1f0…13e082`（16,608,889 B）；含 6 项修复：82bee4e 链条失败续跑、
  2f919ef 识别节选 1500、a0ba7fc 简报公司名编制单位优先、810e4ff 失败原因透出、
  36acd9f+abd839a 识别最多 3 次重试、090dee6 流水日期带时间解析。
- 实机第七轮（用户授权真实模型调用）：阶段 A 真实 5 文件链式——明细表步骤按红线合法拒绝
  且原因透出（"存在多份同类资料"），链条续跑简报成功（docx/pdf/png，三期 5.07/4.76/4.75 万元，
  公司名正确）；阶段 B 明细表专项——detail_workbook.xlsx 真实产出，银行存款 招商银行
  47,499.02 元与流水证据一致，基准日 2026-06-30。
- 冒烟 PASS；PYZ 提取验证 4 处修复标记在包；敏感扫描管理员密码 0 命中；
  回归 1204+10 passed（4 个既有收集错误文件照旧排除）。
- 证据 D:\ZQ-Acceptance\real-machine-test\20260919-dialogfix\（round7.log、data10、shots/70-75）。
- 已知残留：服务端 2048 token 预算致偶发识别截断（客户端 3 次重试后约 1.5%），
  根因修复需 Zeabur 服务端部署，未授权。
- 未推送、未部署、未发布。
