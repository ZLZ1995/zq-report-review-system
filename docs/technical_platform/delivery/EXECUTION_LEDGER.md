...
| G08-03 | doing | 新增签名清单、包哈希、协议/schema范围、平台架构、可信key ID和发布说明；私钥放受控签名环境/GitHub受保护Secret，不进服务端普通变量或仓库；建立轮换/撤销机制。 | `v0.2.7` sequence 2 的签名和内置公钥复验已通过，私钥仅在用户授权的 `D:/1/KEY`。因后续跨进程资源修复，0.2.7 草稿不再作为最终稳定候选；已将客户端提升为 `0.2.8` sequence 3，待精确提交 CI 包生成后离线签名和复验。 |
...
| G08-06 | doing | 发布到GitHub Release或经确认HTTPS分发源；包名和版本不可变，旧URL不覆盖不同内容；私有源采用安全下载机制，客户端无仓库PAT。 | 历史公共 Release `v0.2.7` 的不可变资产和公开下载已验证；最终 `v0.2.8` 尚待精确提交 CI、签名及公共 Release 资产提升。发布工作流已改为按标签动态校验文件名，仍只允许指定 CI 的四项产物。 |
...
| G10-03 | doing | 用验收的同一提交重建EXE及压缩包；保留老稳定版，不覆盖运行中的EXE；启动截图、文件版本、资源清单、SHA256、签名与Git标签一致。 | 历史 `v0.2.7` 的 CI/签名/Release 一致性已验证，但该包不包含后续跨进程资源门禁，不能作为最终候选。已提升 `0.2.8` sequence 3 并补发布说明，待精确提交 Windows CI、包哈希、签名、标签、Release 和打包 EXE 冒烟全部一致后重新签收。 |
| G10-04 | doing | 灰度机器从旧版实际更新到新版本；验证登录、历史、分支、真实流式、取消、浏览器密码和成果；成功后再将该已验收版本设stable。 | `v0.2.7` sequence 2 草稿签名校验通过但未激活；因其不含最新跨进程门禁，稳定通道继续保持 0.2.6。最终改为验收 `v0.2.8`；账号 `001` 的真实模型流式/取消/账务、托管升级与浏览器密码/成果复核完成后才可激活 stable。 |
